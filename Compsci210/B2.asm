...
;turns ascii value into integer
single		ldr	r3, r5, #0		
		ld	r2, ascii
		not	r2, r2
...
;tests if single value is greater than three and not negative	
		and	r4, r4, #0
		add	r4, r3, #0
		brn	main			;branches if value is negative	
		add	r4, r4, #-3
		brn	main			;value less than 3 so prompt again
		brzp	fibonacci

;this section of code came from lecture13 under tentimes.asm
;I have altered it and recommented it.
multi		ldr	r3, r5, #0		; load r3 with the digit to multiply 
		...
		add	r3, r3, r3		;value multiplyed by 2
		add	r4, r3, #0		;multiplyed value now in r4
		add	r3, r3, r3		;value multiplyed by 4
		add	r3, r3, r3		;value multiplyed by 8
		add	r3, r3, r4		;value multiplyed by 10

;adding second digit to the first digit 
;(which has been multiplyed by ten)
...
;tests to see if value less than 24 and not negative 
		and	r4, r4, #0
		add	r4, r3, #0
		brn	main			;braches to input if negative value
		add	r4, r4, #-10
		add	r4, r4, #-13
		brp	main			;if input over 23 then prompt again

;beginning of iterative function
;using the code a,b = b, a+b	
;r0 - b
;r1 - a
;r3 - counter
;r4 - temp value		
fibonacci	and	r1, r1, #0		;starting value for a is 0 
		and	r0, r0, #0	
		add	r0, r0, #1		;starting value for b is 1
		and	r4, r4, #0		;clearing temp number register
		st	r3, counter	
		st	r1, aValueStorage		
		st	r4, tempValueStore	;storeing as I need registers for printing
		st	r0, bValueStorage
		brnzp	print

fibloop		ld	r3, counter		;second time it goes into the loop 
		ld	r1, aValueStorage
		ld	r4, tempValueStore
		ld	r0, bValueStorage
		add	r4, r1, #0		;store a in temp
		add	r1, r0, #0		;store b in a
		add	r0, r0, r4		;add temp to b
...
buffer		.blkw 10
counter		.fill x0000
aValueStorage	.fill x0000
tempValueStore	.fill x0000
bValueStorage	.fill x0000
ascii		.fill x0030
space		.stringz " "
