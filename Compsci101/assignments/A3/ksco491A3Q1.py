""" COMPSCI 101 S1 C, 2015
	...
	authored by Kieran Scott, Upi: ksco491, ID: 439868967 
	version: 29/04/2015
"""
...
def display_books(books_list):
	...
	display_separator()
	for i in books_list:
		display_book = display_a_book(i)
		print("*" * 24)

# This function processes the searching of a book based on an input book title.	
def search_book(books_list):
	book_title_to_search_for = input("Enter the book title to search: ")
	book_finder = find_book(books_list, 1, book_title_to_search_for)
	if book_finder == -1:	
		print("Sorry, this book is not in the collection.") 
	else:
		print("Record found in the collection:")
		display_a_book(books_list[book_finder])
	display_separator()	

# This function processes the borrowing of a book based on the input book code.
def borrow_book(books_list):
	book_code_input = input_code(books_list)
	book = books_list[book_code_input]
	book_list = book.split(",")
	due_date = get_due_date()
	if len(book_list) <= 2:
		print("The book - '", book_list[1], "' is available.", sep = "")
		ask_for_upi = input("Enter the UPI to borrow: ")
		print("The book - '", book_list[0], "' is borrowed by ", ask_for_upi, ".", sep = "")
		print("Due date for returning the book is:", due_date)
		updated_book_status = update_status(books_list, book_code_input, ask_for_upi, due_date)
	else:
		print("Sorry, the book '", book_list[1], "' is on loan.")
		print("It will be returned by ", due_date, ".", sep = "")
	display_separator()
...
def return_book(books_list):
	...
	book_list = book.split(",")
	if len(book_list) > 2:
		print("Thank you for returning the book - '", book_list[1], "'.", sep = "")
		status_update = update_status(books_list, book_code_input, [], [])
	else:
		print("The book - '", book_list[1], "' has not been borrowed.", sep = "")
	display_separator()

# This function displays the borrowing records based on an input UPI.
def display_borrowing(books_list):
	ask_for_upi = input("Enter the UPI to retrieve: ")
	sum = 0
	for i in books_list:
		book_list = i.split(",")
		if len(book_list) > 2 and ask_for_upi == book_list[2]:
			display_a_book(i)
			sum += 1
			print("*" * 24)
	if sum == 0:
		print("There is no record of books borrowed by", ask_for_upi, ".", sep = "")
	elif sum == 1:
		print("There is ", sum, " book borrowed by ", ask_for_upi, ".", sep = "") 
	else:
		print("There are ", sum, " books borrowed by ", ask_for_upi, ".", sep = "")
...
def save_books(books_list, filename):
	changes_to_books = open(filename, "w")
	for i in books_list:
		book_info = i.split(",")
		changes_to_books.write(book_info[0])
		changes_to_books.write(",")
		changes_to_books.write(book_info[1])
		if len(book_info) > 2:
			changes_to_books.write(",")
			changes_to_books.write(book_info[2])
			changes_to_books.write(",")
			changes_to_books.write(book_info[3])
		changes_to_books.write("\n")
	changes_to_books.close()
